import React, {
  ...
  const addContacts = forwardRef(({ route, navigation }, ref) => {
    const { addedPeople,groupid } = route.params;
    ...
    const handleConfirm = () => {
      ...
      if (contactNumber.startsWith("+")) {
        if (checkCode(selectedCountryCode, cleanedPhoneNumber)) {
          ...
          setNewlyAddedContacts((prevAddedContacts) => [...prevAddedContacts, contact]); 
        } else {
    ...
    const handleRemovePersonFromAddPeople = useCallback((index) => {
      setSelectedPeople((prevSelected) => {
        const updatedSelectedPeople = prevSelected.filter((_, i) => i !== index);
    ...
    const renderItem = useCallback(
      ({ item }) => (
        <View style={styles.allPeopleItem}>
          ...
          <TouchableOpacity
            onPress={() => handleAddPersonToSelectedPeople(item)}
          >
    ...
    const handleConfirmButton = async () => {
      await http.put("/addGroup/contacts/", {
        peoples: newlyAddedContacts,
        groupid
      });
      navigation.goBack();
    };
